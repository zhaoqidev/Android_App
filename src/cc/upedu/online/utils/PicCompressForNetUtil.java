 ...
 */
public class PicCompressForNetUtil {
	 ...
	 */
	private static final int MAX_MEMORY = (int) (Runtime.getRuntime()
			.maxMemory() / 1024);// 方案二.以容量为基准
	private static int picMaxSize = MAX_MEMORY / 8 ;
	public static String getCompressBitmap(String filePath, int width) {
		...
		if (file.getName().toLowerCase(Locale.US).contains(".png")) {
			...
			if (file.length() > picMaxSize) {
				options.inSampleSize = PicCompressForNetUtil.calculateInSampleSize(
						options, width, type);
			} else {
...
//			byte[] b = baos.toByteArray();
			try {
				...
				String dir = OnlineApp.myApp.context.getFilesDir().getPath()+OnlineApp.myApp.context.getPackageName()+"/shared_prefs/";
...
//				baos.writeTo(out);
			...
			Bitmapcontent = OnlineApp.myApp.context.getFilesDir().getPath()+OnlineApp.myApp.context.getPackageName()+"/shared_prefs/"+filePath.substring(filePath.lastIndexOf("/")+1);
...
//			Bitmapcontent = Base64.encodeToString(b, Base64.DEFAULT);
	...
	public static String bitmapRateHandler(String filePath,Bitmap bitmap, int type) {
		...
		try {
			String name = filePath.substring(filePath.lastIndexOf("/")+1);
			String dir = "/data/data/"+OnlineApp.myApp.context.getPackageName()+"/shared_prefs/";
...
//		String result = Base64.encodeToString(b, Base64.DEFAULT);
		return "/data/data/"+OnlineApp.myApp.context.getPackageName()+"/shared_prefs/"+filePath.substring(filePath.lastIndexOf("/")+1);
	}
