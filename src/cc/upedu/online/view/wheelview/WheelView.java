...
public class WheelView extends View {
	...
	/** 项目文本颜色 */
	private static final int ITEMS_TEXT_COLOR = 0xff969696;
	...
	/** 标签抵消 */
	private static final int LABEL_OFFSET = 16;
	...
	private void initResourcesIfNecessary() {
		if (itemsPaint == null) {
			...
			itemsPaint.setTextSize(TEXT_SIZE-3);
		}
		...
		if (textPaint == null) {
			textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG
					| Paint.FAKE_BOLD_TEXT_FLAG | Paint.DITHER_FLAG);
			// valuePaint.density = getResources().getDisplayMetrics().density;
			textPaint.setTextSize(TEXT_SIZE-5);
			textPaint.setFakeBoldText(false);
			textPaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFC0C0C0);
		}
	...
	private void createLayouts(int widthItems, int widthLabel) {
		...
		if (widthLabel > 0) {
			if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
				labelLayout = new StaticLayout(label, textPaint, widthLabel,
	...
	private void drawValue(Canvas canvas) {
		...
		textPaint.setColor(ITEMS_TEXT_COLOR);
		...
		if (labelLayout != null) {
			canvas.save();
			canvas.translate(itemsLayout.getWidth() + LABEL_OFFSET, bounds.top + 3);
