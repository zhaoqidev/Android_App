...
import cc.upedu.online.utils.DownloadUtils;
/**
 * 更新APP的servise
...
//          mNotification.contentView.setTextViewText(R.id.app_upgrade_progresstext, progress + "%");
        	mNotification.setLatestEventInfo(DownloadService.this, "成长吧",  progress + "%",mPendingIntent);
...
//            mNotification.contentView.setTextViewText(R.id.app_upgrade_progresstext, "下载完成。");
            mNotification.setLatestEventInfo(DownloadService.this, "成长吧",  "下载完成",mPendingIntent);
            mNotificationManager.notify(mNotificationId, mNotification);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
            destDir = new File(Environment.getExternalStorageDirectory(),"app/download/");
            if (destDir.exists()) {
...
//        mNotification.contentView = new RemoteViews(getApplication().getPackageName(), R.layout.app_upgrade_notification);
        ...
        completingIntent.setClass(getApplication().getApplicationContext(), DownloadService.class);

        mPendingIntent = PendingIntent.getActivity(DownloadService.this, R.string.app_name, completingIntent, PendingIntent.FLAG_UPDATE_CURRENT);
...
//        mNotification.contentView.setTextViewText(R.id.app_upgrade_progresstext, "0%");
        ...
        mNotification.setLatestEventInfo(DownloadService.this, "成长吧",  "1%",mPendingIntent);
        mNotificationManager.cancel(mNotificationId);
        mNotificationManager.notify(mNotificationId, mNotification);
